...
public function CompletedAssessment(Request $request){
  $pageConfigs = ['bodyCustomClass' => 'app-page'];
//        echo '<pre>';
//        print_r($getCourseDetail);
//        die;
  ...
  if (is_object($getUserData) && $getUserData->roles == '3' && $getUserData->status == 'Active') {
      ...
      if (isset($getStudents)) {
          ...
          foreach ($getStudents as $key => $getStudentsVal) {
              if ($i == 0) {
                  ...
                  $status =1;
                  $getAssessment= Assessment::where(['student_id'=>$key,'course_id' => $courseID,'status'=>$status])->get();
              }
    ...
    return view('assessment.CompletedAssessment', ['pageConfigs' => $pageConfigs, 'getStudents' => $getStudents, 'getAssessment' => $getAssessment, 'data' => $data]);


}

public function CompletedAssessmentView(Request $request){
  $pageConfigs = ['bodyCustomClass' => 'app-page'];
  $data = $courseID = '';
  $studentID = $request->studentID;
  $courseID = $request->course;
  ...
  if (is_object($getUserData) && $getUserData->roles == '3' && $getUserData->status == 'Active') {
      ...
      if (isset($getStudents)) {
          ...
          foreach ($getCourseDetail as $keyVal => $val) {
            ...
            $status =1;
          $getAssessment = Assessment::where(['student_id' => $studentID, 'course_id' => $courseID,'status' =>$status])->get();
      }
  }

  $view= view('assessment.completedView', ['pageConfigs' => $pageConfigs, 'getStudents' => $getStudents, 'getAssessment' => $getAssessment, 'data' => $data])->render();
   return $view;
}
}
