...
//        die;
            ...
            if (isset($getStudents)) {
                ...
                foreach ($getStudents as $studKey => $getStudentsVal) {
                    if ($i == 0) {
                        ...
                        $getCourseDetail = StudentCourseTutor::with('getCourseDetail')->where(['student_id' => $studKey, 'tutor_id' => $userID])->get();
                        ...
                        $searchDate = date('Y-m-01 00:00:00');
                        $searchEndDate = date('Y-m-t 23:59:59');
                        $getsessionData = StudentSession::with('getStudHWAttachmentDetail')->where(['student_id' => $studKey, 'course_id' => $courseID, 'status' => 'Active'])
                                ->where('session_date', '>=', $searchDate)->where('session_date', '<=', $searchEndDate)
                                ->orderBy('session_date', 'Desc')
                                ->get();
                    }
        ...
        return view('homework.tutorindex', ['pageConfigs' => $pageConfigs, 'getStudents' => $getStudents, 'getsessionData' => $getsessionData, 'data' => $data, 'timePeriod' => $timePeriod]);
    }
